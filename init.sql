...
CREATE TABLE IF NOT EXISTS problems (
    ...
    category VARCHAR(50) COMMENT '题目类型：动态规划、贪心、搜索、数学、字符串、数据结构、图论、模拟等',
...
SELECT 'A+B Problem', 'Calculate the sum of two integers.', 1000, 128, '/home/oj/data/1', '模拟'
WHERE
    NOT EXISTS (
        SELECT 1
        FROM problems
...
SELECT '素数判断', '给定一个正整数 n，判断它是否为素数。\n\n输入：一个整数 n（2 <= n <= 10^9）\n输出：是素数输出 "Yes"，否则输出 "No"', 1000, 128, '/home/oj/data/5', '数学'
WHERE
    NOT EXISTS (
        SELECT 1
        FROM problems
