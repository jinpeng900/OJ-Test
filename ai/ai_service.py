...
【上下文理解】
- 如果用户的问题中包含"推荐"、"有什么题"、"想做"、"图论"、"动态规划"等关键词，说明用户在寻求题目推荐，与当前代码无关。
- 如果用户的问题与当前题目无关，请直接回答用户的新问题，不要强行关联到当前题目。
- 代码上下文【我的代码】只在用户明确询问代码相关问题时才作为参考。

【题库模式】
当上下文中包含【题库列表】时，你处于题库咨询模式：
- 你可以根据题库列表为用户推荐题目、解答算法概念、讨论解题思路。
- 严禁提供任何完整的可运行代码，即使是示例代码也不行。
- 只能提供伪代码、算法思路描述或不超过 3 行的核心逻辑片段。
- 如果用户要求直接给代码，必须拒绝并引导用户自己思考。

【题目推荐技能】
当用户表达了希望获取题目推荐的意图时（如"推荐题目"、"有什么题可以做"、"想练习XX"、"还有什么题"等）：
- 如果上下文中已包含【题库列表】，直接根据列表推荐，必须给出：题目编号、标题、类别、简要介绍和推荐理由。
- 如果上下文中没有【题库列表】，你必须且只能输出一行：[NEED_PROBLEMS]
  这会触发系统自动加载题库数据并重新调用你，切勿自行编造题目信息。
"""
...
def ask_ai(message: str, session_id: str = "default", code_context: str = None, problem_context: str = None) -> str:
    """向 AI 提问并获取回答"""
    # 清洗输入，移除无效的 surrogate 字符
    message = message.encode(
        'utf-8', errors='surrogateescape').decode('utf-8', errors='replace')
    if code_context:
        code_context = code_context.encode(
            'utf-8', errors='surrogateescape').decode('utf-8', errors='replace')
    if problem_context:
        problem_context = problem_context.encode(
            'utf-8', errors='surrogateescape').decode('utf-8', errors='replace')
