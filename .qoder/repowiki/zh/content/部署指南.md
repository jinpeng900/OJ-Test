...
## 更新摘要
**所做更改**   
- 新增数据库主机配置环境变量 OJ_DB_HOST 的详细说明
- 更新数据库连接配置与权限设置章节
- 增强容器化部署中的数据库连接安全性说明
- 完善故障排查指南中的数据库连接问题解决方案
...
### 组件 E：数据库主机配置与连接管理
- 环境变量配置
  - OJ_DB_HOST：数据库主机名，默认值为 localhost，Compose 中指向 oj-db 服务
  - OJ_DB_PORT：数据库端口，默认 3306
  - 支持自定义数据库主机地址，便于容器化部署中的数据库分离
- 连接工厂实现
  - AppContext::getDbHost() 优先使用环境变量 OJ_DB_HOST
  - 未设置时回退到 localhost，确保向后兼容
- 安全连接模型
  - 数据库用户 oj_admin 和 oj_user 均使用 '%' 主机模式，支持容器间通信
  - 权限分离：管理员全权限，普通用户受限权限
  - 行级隔离：通过应用程序控制用户数据访问范围

```mermaid
flowchart TD
A["应用启动"] --> B["读取环境变量 OJ_DB_HOST"]
B --> C{"OJ_DB_HOST 是否设置"}
C --> |是| D["使用指定主机地址"]
C --> |否| E["使用默认 localhost"]
D --> F["建立数据库连接"]
E --> F
F --> G["创建管理员连接"]
F --> H["创建用户连接"]
...
**图表来源**
- [src/app_context.cpp:6-10](file://src/app_context.cpp#L6-L10)
- [docker-compose.yml:57](file://docker-compose.yml#L57)
- [init.sql:70-87](file://init.sql#L70-L87)

**章节来源**
- [src/app_context.cpp:6-22](file://src/app_context.cpp#L6-L22)
- [docker-compose.yml:55-58](file://docker-compose.yml#L55-L58)
- [init.sql:67-90](file://init.sql#L67-L90)
...
- 数据库连接失败
  ...
  - **新增**：检查 OJ_DB_HOST 是否正确指向数据库服务，或是否需要自定义主机地址
- DinD 权限不足
  - 现象：评测功能不可用
...
- 安全
  ...
  - 使用只读挂载与非特权用户运行
  - 严格管理敏感环境变量与 .env 文件
  - **新增**：合理配置 OJ_DB_HOST 环境变量，确保数据库连接安全性
- 网络
...
- 部署前
  ...
  - 准备测试数据与工作区目录
  - **新增**：配置 OJ_DB_HOST 环境变量（如使用外部数据库）
- 部署后
  - 检查 oj-db 健康与初始化状态
  - 检查 oj-app 启动日志与 AI 配置提示
  - 验证 DinD 权限与沙箱镜像可用性
  - **新增**：验证数据库连接配置（OJ_DB_HOST/OJ_DB_PORT）
  - 执行一次评测与登录测试
