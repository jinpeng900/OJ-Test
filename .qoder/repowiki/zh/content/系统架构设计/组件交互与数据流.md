...
## 更新摘要
**变更内容**
- 更新了角色分离架构的详细分析，反映新的管理员与用户视图分离设计
- 新增了数据库连接建立时机的精确说明，区分管理员和用户的不同连接策略
- 完善了业务逻辑调用链的分析，突出依赖注入和接口抽象的设计原则
- 更新了数据流图，更好地体现新的组件交互模式
- 增强了异步处理、并发控制与事务管理的分析框架
...
### 数据库连接建立时机与生命周期
- **连接建立策略**：管理员视图与用户视图在其start()中分别以不同账号建立数据库连接，体现了明确的角色分离。
- **连接类型差异**：
  - 管理员连接：使用oj_admin账号，具有数据库全权限，适用于题目管理和发布操作。
  - 用户连接：使用oj_user账号，具有受限权限，仅允许查询题目、提交代码和修改个人信息。
- **依赖注入模式**：业务对象通过构造函数接收DatabaseManager指针，实现依赖注入。
- **生命周期管理**：视图退出时显式重置智能指针，确保析构释放连接资源。

```mermaid
sequenceDiagram
participant AV as "AdminView"
participant UV as "UserView"
participant DBA as "DatabaseManager(管理员)"
participant DBU as "DatabaseManager(用户)"
...
### 业务逻辑调用链
- **管理员调用链**：AdminView接收用户输入，调用Admin对象方法；Admin通过DatabaseManager执行SQL。
- **用户调用链**：UserView接收用户输入，调用User对象方法；User通过DatabaseManager执行查询/写入。
- **角色分离优势**：不同的视图层对应不同的数据库权限，确保操作的安全性和职责明确。
...
- **角色分离**：通过不同的视图类和数据库连接策略，实现了明确的职责分离和权限控制。
...
- **并发控制**：
  - 数据库层面：采用"单一受限账号+应用层行级隔离"的策略，通过WHERE条件限定用户可见范围，避免跨用户数据泄露。
  - 应用层面：每个用户会话独立建立连接，避免连接级别的并发冲突。
...
- **权限检查优化**：应用层行级隔离已在UserView中实现，可进一步优化WHERE条件的性能。

## 故障排查指南
- **登录菜单输入异常**：检查输入缓冲区清理逻辑与异常分支处理。
- **数据库连接失败**：
  - 确认MySQL服务状态
  - 检查管理员账号oj_admin和用户账号oj_user的凭据正确性
  - 核对init.sql中权限授予情况
...
## 结论
本系统通过清晰的三层架构实现了良好的职责分离：视图层负责交互与流程控制，业务层封装领域逻辑，数据访问层屏蔽底层细节。数据库连接按需建立并在会话结束时释放，配合应用层行级隔离策略满足基本安全需求。新的角色分离设计进一步强化了系统的安全性，通过不同的数据库权限和视图层实现了明确的职责边界。建议后续引入连接池、事务管理与异步处理机制以进一步提升性能与可靠性。

## 附录
- **初始化脚本**：init.sql定义了数据库结构、示例数据与用户权限，可用于快速搭建测试环境。
- **颜色常量**：color_codes.h提供ANSI颜色常量，用于美化命令行输出。
- **权限设计**：oj_admin拥有数据库全权限，oj_user仅具备必要的受限权限，体现了最小权限原则。
