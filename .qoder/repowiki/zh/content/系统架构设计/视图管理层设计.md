...
subgraph "主控制器"
VM["ViewManager<br/>登录菜单/角色切换<br/>程序入口管理"]
end
subgraph "视图层"
...
```mermaid
flowchart TD
Start(["进入start_login_menu<br/>(主控制器)"]) --> Clear["清屏"]
...
## 结论
ViewManager模块通过清晰的职责划分与稳健的输入处理，实现了OJ系统的**真正主控制器**角色。作为程序唯一入口点，ViewManager负责统一管理程序入口、角色选择与菜单导航，AdminView与UserView分别承载管理员与用户业务，结合DatabaseManager提供的数据访问能力，形成了稳定的CLI架构。该设计易于扩展新的角色或功能，同时保持良好的可维护性与用户体验。
