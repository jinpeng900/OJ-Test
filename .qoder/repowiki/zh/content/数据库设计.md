...
## 更新摘要
**变更内容**
- 新增环境变量数据库主机配置支持，增强容器化部署灵活性
- 扩大数据库用户权限范围，从localhost到%，提升容器间通信能力
- 保持表结构PRIMARY KEY位置不变，维持现有数据模型稳定性
...
- 应用上下文：集中管理数据库连接（管理员与受限用户两种角色），支持环境变量配置。
...
- AppContext：提供管理员与受限用户两类连接工厂，分别对应不同权限的数据库用户，支持环境变量配置数据库主机地址。
...
## 架构总览
- 连接管理：通过 AppContext 统一创建连接，区分管理员与受限用户；受限用户复用同一数据库账号，行级隔离由应用层在 SQL 中强制实现。支持通过环境变量 OJ_DB_HOST 配置数据库主机地址，增强容器化部署灵活性。
- 权限模型：oj_admin 拥有全权限；oj_user 仅授予对 problems 的只读、对 users 的插入/查询/更新、对 submissions 的插入/查询。用户权限范围从 localhost 扩展到 %，支持容器间通信。
...
### 连接管理与权限控制
- 连接工厂：AppContext 提供 createAdminDB 与 createUserDB，分别连接 oj_admin 与 oj_user。支持通过环境变量 OJ_DB_HOST 配置数据库主机地址，默认回退到 localhost。
- 权限分配：
...
- **更新**：数据库用户权限范围从 localhost 扩展到 %，支持容器间通信，增强部署灵活性。
...
### 环境变量配置与容器化支持
- **新增**：通过环境变量 OJ_DB_HOST 配置数据库主机地址，支持容器化部署场景。
- 配置方式：
  - docker-compose.yml 中设置 OJ_DB_HOST: oj-db
  - docker-entrypoint.sh 中使用 ${OJ_DB_HOST:-localhost} 作为默认值
  - AppContext::getDbHost() 优先使用环境变量，回退到 localhost
- **更新**：数据库用户权限从 localhost 扩展到 %，支持容器间网络通信。

**章节来源**
- [docker-compose.yml:56-57](file://docker-compose.yml#L56-L57)
- [docker-entrypoint.sh:29](file://docker-entrypoint.sh#L29)
- [app_context.cpp:6-10](file://src/app_context.cpp#L6-L10)
- [init.sql:70-77](file://init.sql#L70-L77)
...
## 结论
本数据库设计围绕用户、题目、提交三大核心实体构建，采用行级隔离与最小权限原则，结合应用层的查询与更新策略，实现了基本的在线评测功能。**更新后**的版本通过环境变量配置增强了容器化部署的灵活性，通过扩大用户权限范围提升了容器间通信能力，建议后续引入事务、连接池与缓存，完善索引与查询优化，以提升一致性、性能与可维护性。

## 附录
- 初始化脚本与示例数据：见 init.sql。
- Docker 编排与健康检查：见 docker-compose.yml。
- 查询与历史管理设计：见 docs/code_submission_design.md。
- **新增**：环境变量配置文档：见 docs/async_concurrent_judge_architecture.md。