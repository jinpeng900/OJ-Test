...
- 视图层：AdminView 负责管理员菜单与输入处理；ViewManager 统一调度登录入口与角色切换；UserView 负责用户界面。
...
- UserView：负责用户界面展示与用户操作处理，建立数据库连接并驱动 User 执行具体操作。
...
### 用户管理功能概述
虽然现有文档主要关注管理员的题目管理功能，但系统还提供了完整的用户管理功能：
- 用户注册与登录：UserView 提供用户注册和登录功能，支持密码哈希验证。
- 题目浏览：用户可以查看题目列表、查看详情、提交代码等。
- 提交管理：用户可以查看自己的提交历史并修改密码。
- 权限控制：通过 oj_user 账户实现受限权限，应用层通过行级隔离控制用户访问。

**章节来源**
- [src/user_view.cpp:17-109](file://src/user_view.cpp#L17-L109)
- [src/user_view.cpp:138-214](file://src/user_view.cpp#L138-L214)
- [include/user_view.h:11-83](file://include/user_view.h#L11-L83)
...
系统还提供了完整的用户管理功能，通过不同的数据库用户权限实现角色分离，UserView 负责用户界面操作，User 类处理用户业务逻辑，形成了完整的在线判题系统架构。
