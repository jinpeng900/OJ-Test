...
// ----------------------------------------------------------------
// 工作区管理：为每个用户建立独立的 workspace/<id>/ 目录
// ----------------------------------------------------------------
static void ensure_workspace(int user_id)
{
    std::string dir = "workspace/" + std::to_string(user_id);
    mkdir(dir.c_str(), 0755); // 目录已存在时静默跳过

    std::string cpp_path = dir + "/solution.cpp";
    std::ifstream check(cpp_path);
    if (!check.good())
    {
        // 初始化模板文件
...
bool User::login(const string &account, const string &password)
{
    ...
    ensure_workspace(current_user_id); // 确保用户工作区存在
...
bool User::register_user(const string &account, const string &password)
{
    ...
    string insert_sql = "INSERT INTO users (account, password_hash) VALUES ('" +
                        escaped_account + "', '" + password_hash + "')";

    if (db_manager->run_sql(insert_sql))
    {
        // 获取新用户 ID 并创建工作区
        auto id_result = db_manager->query("SELECT LAST_INSERT_ID() AS id");
        if (!id_result.empty())
        {
            int new_id = stoi(id_result[0]["id"]);
...
void User::submit_code(int problem_id, const string &code, const string &language)
{
    ...
    // 路径回退：DB 路径不可访问时尝试本地 ../data/<id>
    if (access(test_data_path.c_str(), R_OK) != 0)
    {
        string fallback = "../data/" + to_string(problem_id);
        if (access(fallback.c_str(), R_OK) == 0)
            test_data_path = fallback;
    }
    ...
    case JudgeResult::RUNTIME_ERROR:
        ...
        break;
    case JudgeResult::COMPILE_ERROR:
        cout << YELLOW << "⚠️  Compile Error (CE)" << RESET << endl;
        cout << RED << "\n编译错误信息:\n"
...
void User::view_my_submissions()
{
    ...
    for (const auto &row : results)
    {
        ...
        // 手动补齐（ANSI 代码影响 setw）
        int pad = 8 - static_cast<int>(status.length());
        if (pad > 0)
            cout << string(pad, ' ');
