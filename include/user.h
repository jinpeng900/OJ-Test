...
public:
    ...
    /**
     * @brief 获取最近一次评测的错误上下文（供 AI 分析）
     */
    std::string getLastErrorContext() const { return last_error_context_; }

    /**
     * @brief 获取最近一次评测报告
     */
    JudgeReport getLastReport() const { return last_report_; }
...
private:
    ...
    JudgeReport last_report_{};      // 最近一次评测结果缓存
    std::string last_error_context_; // 评测错误上下文（供 AI 使用）
};
